...
const PublisherLogin = () =>{
    ...
    const handleLogin = async () => {
        try {
            const res = await axios.post("http://localhost:3000/publisher/signin", {publisherEmail : email, publisherPassword : password})
            if (res) {
            ...
            <Card className="card-global-style" style={{ height: '500px', width: '800px', borderRadius: '20px', display: 'flex', justifyContent: 'center' }}>
                ...
                <Card className="card-global-style" style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <Typography variant="h5">PUBLISHER LOGIN</Typography>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', padding: '5px' }}>
                        <Typography variant="body2">UserLogin Login</Typography>
                        <Typography variant="body2">Publisher Signup</Typography>
                    </div>
                </Card >
            </Card>
        </div >
